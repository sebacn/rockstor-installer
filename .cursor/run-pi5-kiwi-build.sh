...
nohup run_root bash -c "docker run --name '$CONTAINER_NAME' --privileged --cap-add SYS_ADMIN \
  ...
  -w /workspace \
  '$IMAGE' \
  sudo bash -c 'set -e; zypper --non-interactive in -y util-linux util-linux-systemd libpam-pwquality device-mapper kpartx parted systemd; command -v lsblk >/dev/null; modprobe loop max_part=8 2>/dev/null || true; exec kiwi-ng --shared-cache-dir=/kiwi-package-cache --profile=Tumbleweed.RaspberryPi5 --type oem system build --description ./ --target-dir /home/kiwi-images/' \
