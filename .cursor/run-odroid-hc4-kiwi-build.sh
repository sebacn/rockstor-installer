...
run_root docker run -d --name "$CONTAINER_NAME" --privileged --cap-add SYS_ADMIN \
  ...
  -w /workspace \
  "$IMAGE" \
  sudo bash -c 'zypper --non-interactive in -y util-linux device-mapper kpartx parted systemd >/dev/null 2>&1 || true; modprobe loop max_part=8 2>/dev/null || true; exec kiwi-ng --profile='"$PROFILE"' --type oem system build --description ./ --target-dir /home/kiwi-images/ --shared-cache-dir=/kiwi-package-cache' \
  >/dev/null
